...
function simulate(
    ...
    for i in 1:steps
        ...
        if (@isdefined island_network)
            ...
            island_network = mainland[
                union(migrating_parasites, resident_parasites),
                union(migrating_hosts, resident_hosts),
            ]
        else
        ...
        # Extinctions
        remaining_hosts =
            filter(sp -> rand() ≥ extinction(island), species(island_network; dims = 2))
        ...
        remaining_parasites = filter(
            sp -> rand() ≥ extinction(island)^d_parasites[sp],
            species(island_network; dims = 1),
        )
...
@info "plotting"
StatsPlots.@df df Plots.scatter(
    :α,
    :hosts,
    legend = :bottomright,
    c = :black,
